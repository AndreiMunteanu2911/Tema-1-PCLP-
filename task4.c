...
void numarare_biti(unsigned int n)
{
	...
	for (int i = 1; i <= N; i++)
	{
		...
		case(1):
		{
			tablou_valori[i] = tablou_valori[i] * tablou_valori[i + 1];
			for (int j = i; j <= N; j++)
			{
				tablou_valori[j] = tablou_valori[j + 1];
				tablou_operatii[j] = tablou_operatii[j + 1];
			}
			nr_inmultiri_impartiri++;
			break;
		}
		case(3):
		{
	...
	for (unsigned int i = 1; i <= (N-nr_inmultiri_impartiri); i++)
	{
		switch (tablou_operatii[i])
		{
		case (0):
